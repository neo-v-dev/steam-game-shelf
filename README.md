...
- **管理ページ**: リスト/カード(サムネイル)表示を切り替えながら、表示/非表示・プレイ済みタグ・時間表示・配信リンク・サイト設定をブラウザから編集可能。
- **非表示設定**: 見せたくないゲームをサイトから除外可能。
- **プレイ済みタグ**: 初期値はプレイ時間から自動判定。サイト上でバッジ表示&絞り込みできる。
- **プレイ時間の表示制御**: サイト全体の一括OFFと、ゲームごとの個別OFFの両方に対応。
- **配信リンク**: ページ上部にチャンネルリンク、各ゲームに配信/動画リンク(▶アイコン)を設定可能。
...
**管理ページを使う方法(推奨)**: `https://<ユーザー名>.github.io/<リポジトリ名>/admin.html` を開き、下記「管理ページ」の手順でアクセストークンを設定すると、チェックボックスで表示/非表示を編集できます。
...
`https://<ユーザー名>.github.io/<リポジトリ名>/admin.html` で、以下をブラウザから編集できます(JSON 編集不要)。**リスト表示**と、公開ページと同じ見た目の**カード(サムネイル)表示**を切り替えられます。

- ゲームごとの**表示/非表示**(リスト: チェックボックス / カード: カードをクリック。検索絞り込み・一括ON/OFFつき)
- ゲームごとの**プレイ済みタグ**(チップをクリック。一括ON/OFFつき)
- ゲームごとの**プレイ時間表示**(「時間表示」チップでOFFにすると、そのゲームだけプレイ時間を隠せる)
- ゲームごとの**配信/動画リンク**(URLを入力すると、公開ページのカードに▶アイコンが表示される)
- サイト設定: **サイト名 / 初期表示言語 / プレイ時間表示(サイト全体) / チャンネルURL**
  - 「プレイ時間を表示(サイト全体)」はいわば一括スイッチで、OFFにしてもゲームごとの設定は保持されます
...
トークンはブラウザの localStorage にのみ保存され、api.github.com 以外には送信されません。共用PCでは使用後にブラウザのサイトデータを削除してください。`admin.html?demo=1` でサンプルデータの操作を試せます。

## プレイ済みタグ

各ゲームに「プレイ済み」タグを付けられます。初回はプレイ時間が1分以上あれば自動でONになり、以降は管理ページ(または `data/catalog.json` の `played`)で自由に変更できます(手動設定は自動更新で上書きされません)。サイトではバッジ表示と「プレイ済み / 未プレイ」絞り込みに使われます。
